...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const [customersRes, contactsRes, dealsRes, invoicesRes, paymentsRes, refundsRes] = await Promise.all([
                    fetch('/api/customers'),
                    fetch('/api/contacts'),
                    fetch('/api/deals'),
                    fetch('/api/invoices'),
                ...
                const unpaidInvoices = invoicesData.filter((i: any) => i.status !== 'Paid');
                const totalInvoiceAmount = invoicesData.reduce((sum: number, inv: any) => sum + inv.totalAmount, 0);
                const totalPaidAmount = paymentsData.filter((p: any) => p.status === 'Completed').reduce((sum: number, p: any) => sum + p.amount, 0);

                const completedPayments = paymentsData.filter((p: any) => p.status === 'Completed');
                const stoppedPayments = paymentsData.filter((p: any) => p.status === 'Stopped');

                const pendingRefunds = refundsData.filter((r: any) => r.status === 'Pending');
                const approvedRefunds = refundsData.filter((r: any) => r.status === 'Approved');
                const completedRefunds = refundsData.filter((r: any) => r.status === 'Completed');
                const totalRefundAmount = completedRefunds.reduce((sum: number, r: any) => sum + r.amount, 0);
    ...
    const paymentStatusData = {
        labels: ['Completed', 'Pending', 'Stopped', 'Failed', 'Cancelled'],
        datasets: [
            {
                label: 'Payment Count',
                data: [
    ...
    return (
        <div className="dashboard">
            <div className="dashboard-header">
                <h1>Dashboard</h1>
                <p className="dashboard-subtitle">Comprehensive overview of your CRM and financial data</p>
            </div>
            ...
            <div className="stats-grid">
                ...
                <div className="stat-card stat-success">
                    ...
                    <div className="stat-content">
                        ...
                        <div className="stat-value">${stats.totalPaidAmount.toLocaleString()}</div>
                        <div className="stat-description">{stats.completedPayments} completed payments</div>
                    </div>
                </div>
                ...
                <div className="stat-card stat-warning">
                    ...
                    <div className="stat-content">
                        ...
                        <div className="stat-value">${(stats.totalInvoiceAmount - stats.totalPaidAmount).toLocaleString()}</div>
                        <div className="stat-description">{stats.unpaidInvoices} unpaid invoices</div>
                    </div>
                </div>
                ...
                <div className="stat-card stat-danger">
                    ...
                    <div className="stat-content">
                        ...
                        <div className="stat-value">${stats.totalRefundAmount.toLocaleString()}</div>
                        <div className="stat-description">{stats.completedRefunds} completed refunds</div>
                    </div>
                </div>
            </div>
                    ...
                    {stats.stoppedPayments > 0 && (
                        <div className="alert-card alert-danger">
                            ...
                            <div className="alert-content">
                                ...
                                <div className="alert-description">Payments have been stopped</div>
                                <Link href="/payments?status=Stopped" className="alert-link">View Details →</Link>
                            </div>
                        </div>
                    )}
                    ...
                    {stats.pendingRefunds > 0 && (
                        <div className="alert-card alert-warning">
                            ...
                            <div className="alert-content">
                                ...
                                <Link href="/refunds?status=Pending" className="alert-link">Review Now →</Link>
                            </div>
                        </div>
                    )}
                    ...
                    {stats.unpaidInvoices > 0 && (
                        <div className="alert-card alert-info">
                            ...
                            <div className="alert-content">
                                ...
                                <Link href="/invoices?status=Sent" className="alert-link">View All →</Link>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="pipeline-card pipeline-open">
                        ...
                        <div className="pipeline-bar">
                            <div 
                                ...
                                style={{ width: `${stats.totalDeals > 0 ? (stats.openDeals / stats.totalDeals) * 100 : 0}%` }}
                            ></div>
                        </div>
                    </div>
                    ...
                    <div className="pipeline-card pipeline-progress">
                        ...
                        <div className="pipeline-bar">
                            <div 
                                ...
                                style={{ width: `${stats.totalDeals > 0 ? (stats.inProgressDeals / stats.totalDeals) * 100 : 0}%` }}
                            ></div>
                        </div>
                    </div>
